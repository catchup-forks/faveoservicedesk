...
class ApiController extends Controller
{
    ...
    public function createTicket(
        \App\Http\Requests\helpdesk\CreateTicketRequest $request,
        \App\Model\helpdesk\Utility\CountryCode $code
    ) {
        try {
    ...
    public function openedTickets()
    {
        try {
            ...
            $result = $this->user->join('tickets', function ($join) {
                ...
                ->select('first_name', 'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'title',
                    'tickets.created_at', 'department.name as department_name',
                    'ticket_priority.priority as priotity_name', 'sla_plan.name as sla_plan_name',
                    'help_topic.topic as help_topic_name', 'ticket_status.name as ticket_status_name')
    ...
    public function unassignedTickets()
    {
        try {
            ...
            $unassigned = $this->user->join('tickets', function ($join) {
                ...
                ->select(\DB::raw('max(ticket_thread.updated_at) as updated_at'), 'user_name', 'first_name',
                    'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'title', 'tickets.created_at',
                    'department.name as department_name', 'ticket_priority.priority as priotity_name',
                    'sla_plan.name as sla_plan_name', 'help_topic.topic as help_topic_name',
    ...
    public function getCustomers()
    {
        try {
            ...
            $result = $this->faveoUser->where('first_name', 'like', '%' . $search . '%')->orWhere('last_name', 'like',
                '%' . $search . '%')->orWhere('user_name', 'like', '%' . $search . '%')->orWhere('email', 'like',
                '%' . $search . '%')->get();
    ...
    public function getCustomersWith()
    {
        try {
            $users = $this->user
                ...
                ->where('role', 'user')
                ->select('users.id', 'user_name', 'first_name', 'last_name', 'email', 'phone_number',
                    'users.profile_pic', 'organization.name AS company', 'users.active')
                ->paginate(10)
                ->toJson();
    ...
    public function searchTicket()
    {
        try {
            ...
            $search = $this->request->input('search');
            $result = $this->thread->select('ticket_id')->where('title', 'like', '%' . $search . '%')->orWhere('body',
                'like', '%' . $search . '%')->get();
    ...
    public function ticketThreads()
    {
        try {
            ...
            $result = $this->user
                ...
                ->select('ticket_thread.id', 'ticket_id', 'user_id', 'poster', 'source', 'title', 'body', 'is_internal',
                    'format', 'ip_address', 'ticket_thread.created_at', 'ticket_thread.updated_at', 'users.first_name',
                    'users.last_name', 'users.user_name', 'users.email', 'users.profile_pic')
                ->where('ticket_id', $id)
                ->get()
                ->toJson();
    ...
    public function inbox()
    {
        try {
            ...
            $inbox = $this->user->join('tickets', function ($join) {
                ...
                ->select(\DB::raw('max(ticket_thread.updated_at) as updated_at'), 'user_name', 'first_name',
                    'last_name', 'email', 'profile_pic', 'ticket_number', 'tickets.id', 'ticket_thread.title',
                    'tickets.created_at', 'department.name as department_name',
                    'ticket_priority.priority as priotity_name', 'sla_plan.name as sla_plan_name',
                    'help_topic.topic as help_topic_name', 'ticket_status.name as ticket_status_name',
                    'department.id as department_id', 'users.primary_dpt as user_dpt')
                ->where(function ($query) use ($user) {
                    if ($user->role != 'admin') {
                        $query->where('tickets.dept_id', '=', $user->primary_dpt);
                    }
...
//                    })
    ...
    public function getMyTicketsUser()
    {
        try {
            ...
            $id = $this->request->input('user_id');
            if ($this->user->where('id', $id)->first()->role == 'admin' || $this->user->where('id',
                    $id)->first()->role == 'agent') {
