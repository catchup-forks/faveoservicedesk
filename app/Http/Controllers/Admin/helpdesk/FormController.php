...
class FormController extends Controller
{
    ...
    public function edit($id)
    {
        try {
            ...
            if ($form) {
                $fields = $form->fields();
                //dd($fields);
                return view('themes.default1.admin.helpdesk.manage.form.edit',
                    compact('form', 'fields', 'select_forms'));
            }
    ...
    public function addChildForm($id)
    {
        try {
            ...
            if ($form) {
                $fields = $form->fields();
                //dd($fields);
                return view('themes.default1.admin.helpdesk.manage.form.add-child',
                    compact('form', 'fields', 'select_forms'));
            }
        ...
        switch ($field_type) {
            ...
            case 'text':
                return Form::label($field->label, $field->label, ['class' => $required_class]) .
                    Form::$field_type($field->name, null,
                        ['class' => "form-control $field->id", 'id' => $field->id, 'required' => $required]);
            case 'email':
                return Form::label($field->label, $field->label, ['class' => $required_class]) .
                    Form::$field_type($field->name, null,
                        ['class' => "form-control $field->id", 'id' => $field->id, 'required' => $required]);
    ...
    public static function selectForm($field_type, $field, $required, $required_class)
    {
        ...
        $form_hidden = Form::hidden('fieldid[]', $field->id,
                ['id' => 'hidden' . $session . $field->id]) . Form::label($field->label, $field->label,
                ['class' => $required_class]);
        $select = Form::$field_type($field->name, [
                '' => 'Select',
                'Selects' => self::removeUnderscoreFromDB($field->values()->lists('field_value',
                    'field_value')->toArray())
            ], null, [
    ...
    public static function radioForm($field_type, $field, $required, $required_class)
    {
        ...
        if (count($values) > 0) {
            foreach ($values as $field_value) {
                $script = self::jqueryScript($field_value, $field->id, $field->name, $field_type);
                $radio .= '<div>' . Form::hidden('fieldid[]', $field->id,
                        ['id' => $field->id . str_slug($field_value)]);
                $radio .= Form::$field_type($field->name, $field_value, null, [
                        'class' => "$field->id",
                        'id' => str_slug($field_value),
                        'required' => $required
                    ]) . $script . '<span>   ' . removeUnderscore($field_value) . '</span></div>';
            }
            $html = Form::label($field->label, $field->label,
                    ['class' => $required_class]) . '</br>' . $radio . '<div id=' . $field->name . '></br></div>';
        }
    ...
    public static function checkboxForm($field_type, $field, $required, $required_class)
    {
        ...
        if (count($values) > 0) {
            ...
            foreach ($values as $field_value) {
                ...
                $checkbox .= Form::$field_type($field->name, $field_value, null,
                    ['class' => "$field->id", 'id' => $session . $i, 'required' => $required]);
