...
class EmailsController extends Controller
{
    ...
    public function create(
        ...
        MailboxProtocol $mailbox_protocol
    ) {
        try {
            ...
            return view('themes.default1.admin.helpdesk.emails.emails.create',
                compact('mailbox_protocols', 'priority', 'departments', 'helps', 'services'));
        } catch (Exception $e) {
    ...
    public function sendDiagnoEmail($request)
    {
        ...
        $service_request = $request->except('sending_status', '_token', 'email_address', 'email_name', 'password',
            'department', 'priority', 'help_topic', 'fetching_protocol', 'fetching_host', 'fetching_port',
            'fetching_encryption', 'imap_authentication', 'sending_protocol', 'sending_host', 'sending_port',
            'sending_encryption', 'smtp_authentication', 'internal_notes', '_wysihtml5_mode');
    ...
    public function edit(
        ...
        try {
            ...
            return view('themes.default1.admin.helpdesk.emails.emails.edit',
                compact('mailbox_protocols', 'priority', 'departments', 'helps', 'emails', 'sys_email',
                    'services'))->with('count', $count);
        } catch (Exception $e) {
            // return if try fails
            return redirect()->back()->with('fails', $e->getMessage());
        }
    ...
    public function validatingEmailSettings(MailRequest $request, $id = '')
    {
        ...
        try {
            $service_request = $request->except('sending_status', '_token', 'email_address', 'email_name', 'password',
                'department', 'priority', 'help_topic', 'fetching_protocol', 'fetching_host', 'fetching_port',
                'fetching_encryption', 'imap_authentication', 'sending_protocol', 'sending_host', 'sending_port',
                'sending_encryption', 'smtp_authentication', 'internal_notes', '_wysihtml5_mode', 'code');
    ...
    public function fetch($email)
    {
        ...
        $TicketController = new \App\Http\Controllers\Agent\helpdesk\TicketController($PhpMailController,
    ...
    public function validateEmailError($out, $in)
    {
        ...
        if ($in !== 1) {
            return $this->jsonResponse('fails',
