...
class ChangesController extends BaseServiceDeskController
{
    ...
    public function getChanges()
    {
        try {
            ...
            $changes = $change->select('id', 'description', 'subject', 'status_id', 'priority_id', 'change_type_id',
                'impact_id', 'location_id', 'approval_id')->get();
            ...
            return \Datatable::Collection($changes)
                ...
                ->orderColumns('description', 'subject', 'reason', 'status_id', 'priority_id', 'change_type_id',
                    'impact_id', 'location_id', 'approval_id')
                ->make();
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    ...
    public function changescreate()
    {
        try {
            ...
            return view('service::changes.create',
                compact('statuses', 'assets', 'sd_changes_priorities', 'sd_changes_types', 'sd_impact_types',
                    'sd_locations', 'users', 'requester'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
    ...
    public function changeshandleCreate(CreateChangesRequest $request, $attach = false)
    {
        ...
        try {
            ...
            \App\Plugins\ServiceDesk\Controllers\Library\UtilityController::attachment($sd_changes->id, 'sd_changes',
                $request->file('attachments'));
            \App\Plugins\ServiceDesk\Controllers\Library\UtilityController::storeAssetRelation('sd_changes',
                $sd_changes->id, $request->input('asset'));
            if ($attach == false) {
    ...
    public function changesedit($id)
    {
        try {
            ...
            return view('service::changes.edit',
                compact('sd_changes_status', 'assets', 'change', 'statuses', 'sd_changes_priorities',
                    'sd_changes_types', 'sd_impact_types', 'sd_locations', 'users', 'requester'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
