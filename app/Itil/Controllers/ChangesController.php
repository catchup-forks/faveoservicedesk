...
class ChangesController extends BaseServiceDeskController
{
    ...
    public function changescreate()
    {
        try {
            ...
            return view('itil::changes.create',
                compact('statuses', 'sd_changes_priorities', 'sd_changes_types', 'sd_impact_types', 'sd_locations',
                    'users', 'requester'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    ...
    public function changeshandleCreate(CreateChangesRequest $request, $attach = false)
    {
        ...
        try {
            ...
            \App\Itil\Controllers\UtilityController::attachment($sd_changes->id, 'sd_changes',
                $request->file('attachments'));
            if (isAsset() == true) {
                \App\Itil\Controllers\UtilityController::storeAssetRelation('sd_changes', $sd_changes->id,
                    $request->input('asset'));
            }
            if ($attach == false) {
    ...
    public function changesedit($id)
    {
        try {
            ...
            return view('itil::changes.edit',
                compact('sd_changes_status', 'change', 'statuses', 'sd_changes_priorities', 'sd_changes_types',
                    'sd_impact_types', 'sd_locations', 'users', 'requester'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }
