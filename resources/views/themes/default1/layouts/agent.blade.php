    ...
    <aside class="main-sidebar">
        ...
        <section class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                ?>
                <?php $myticket = App\Model\helpdesk\Ticket\Tickets::where('assigned_to',
                    Auth::user()->id)->where('status', '1')->get();?>
                <?php $unassigned = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '0')->where('status',
                    '1')->get();
                ...
                $open = App\Model\helpdesk\Ticket\Tickets::where('status', '=', '1')->where('dept_id', '=',
                    $dept->id)->get();
                $open = count($open);
                // dd($open);

                $closed = App\Model\helpdesk\Ticket\Tickets::where('status', '=', '2' || '3')->where('dept_id', '=',
                    $dept->id)->get();
                $closed = count($closed);
                // dd($closed);

                $underprocess = App\Model\helpdesk\Ticket\Tickets::where('status', '=',
                    '2' || '3')->where('assigned_to', '=', '0')->get();
...
<!-- Right side column. Contains the navbar and content of the page -->
                ...
                <div class="tabs-content">
                    ...
                    <div class="tabs-pane @yield('user-bar')" id="tabB">
                        <ul class="nav navbar-nav">
                            ...
                            <li id="bar" @yield('organizations')><a href="{{ url('organizations') }}">Organizations</a>
                            </li>
                            </a></li>
                        </ul>
                    </div>
    ...
    <footer class="main-footer">
        ...
        <strong>Copyright &copy; {!! date('Y') !!} <a
                    href="{!! $company->website !!}">{!! $company->company_name !!}</a>.</strong> All rights reserved.
