...
@section('content')
    ...
    <div id="form-content">
        ...
        <div id="wrapper">
            ...
            <br/><br/>
            <p>NOTE:&nbsp;FAVEO doesn't work properly without JavaScript. In order to install and run FAVEO to its full
                extent please make sure JavaScript is enabled in your browser.</p>
            <?php

            function validate_php(&$results)
            {
                if (version_compare(PHP_VERSION, '5.5') == -1) {
                    $results[] = new TestResult('Minimum PHP version required in order to run Faveo HELPDESK is PHP 5.5. Your PHP version: ' . PHP_VERSION,
                        STATUS_ERROR);
                    return false;
                } else {
            ...
            function checkFilePermission(&$results)
            {
                ...
                if ($f1 >= '644' && $f2 >= '755') {
                    ...
                    if (isset($path1)) {
                        $results[] = new TestResult('File permissions needed.<ul><b>Change file permission for following files</b><li>' . $path1 . '%nbsp: \'644\'</li><li>' . $path2 . '%nbsp: \'755\'</li></ul></br>Change the permission manually on your server or <a href="change-file-permission">click here.</a>',
                            STATUS_ERROR);
                    } else {
                        $results[] = new TestResult('File permissions needed.<ul><b>Change file permission to "755" for following files</b><li>' . $path2 . '</li></ul></br>Change the permission manually on your server or <a href="change-file-permission">click here.</a>',
                            STATUS_ERROR);
                    }
                    return false;
                }
            ...
            function validate_zend_compatibility_mode(&$results)
            {
                ...
                if (version_compare(PHP_VERSION, '5.0') >= 0) {
                    if (ini_get('zend.ze1_compatibility_mode')) {
                        $results[] = new TestResult('zend.ze1_compatibility_mode is set to On. This can cause some strange problems. It is strongly suggested to turn this value to Off (in your php.ini file)',
                            STATUS_WARNING);
                        $ok = false;
                    } else {
            ...
            function validate_extensions(&$results)
            {
                ...
                foreach ($required_extensions as $required_extension) {
                    if (extension_loaded($required_extension)) {
                        ...
                        $results[] = new TestResult("Extension '$required_extension' is required in order to run Faveo Helpdesk ",
                            STATUS_ERROR);
                        $ok = false;
                    } // if
                } // foreach
                ...
                if (extension_loaded('XCache') && ini_get('xcache.cacher')) {
                    $results[] = new TestResult("XCache opcode cache enabled. <span class=\"details\">XCache opcode cache causes Faveo Helpdesk to crash. <a href=\"http://xcache.lighttpd.net/wiki/XcacheIni\">Disable it</a> for folder where Faveo Helpdesk is installed, or use APC instead: <a href=\"http://www.php.net/apc\">http://www.php.net/apc</a>.</span>",
                        STATUS_ERROR);
                    $ok = false;
                } // if
                ...
                foreach ($recommended_extensions as $recommended_extension => $recommended_extension_desc) {
                    if (extension_loaded($recommended_extension)) {
                        ...
                        $results[] = new TestResult("Extension '$recommended_extension' was not found. <span class=\"details\">$recommended_extension_desc</span>",
                            STATUS_WARNING);
                    } // if
                } // foreach
                    ...
                    foreach ($required_functions as $value) {
                        if ($key == $value) {
                            if (strpos(ini_get('disable_functions'), $key) !== false) {
                                $results[] = new TestResult("Function '$value' is required in order to run Faveo Helpdesk. Please check php.ini to enable this function or contact your server administrator",
                                    STATUS_ERROR);
                                $ok = false;
                            } else {
            ...
            function checkMaxExecutiontime(&$results)
            {
                ...
                if ((int)ini_get('max_execution_time') >= 120) {
                    ...
                    $results[] = new TestResult("Maximum execution time is too low. Recommneded execution time is 120 seconds ",
                        STATUS_WARNING);
                }
                return $ok;
            }
        ...
        <form action="{{URL::route('postprerequisites')}}" method="post" class="border-line">
            ...
            <p class="setup-actions step">
                <input type="submit" id="submitme" class="button-primary button button-large button-next"
                       value="Continue">
                <a href="{!! route('licence') !!}" class="button button-large button-next"
    ...
    <p class="setup-actions step">
        <a href="{{URL::route('licence')}}" style="float: left">
            <button value="prev" class="button-primary button button-large">Previous</button>
        </a>
        <button disabled="" class="button-primary button button-large button-next" style="float: right">Continue
        </button>
    </p> <?php
    }
    ?>
    ...
    <div id="legend">
        ...
        <p><span class="ok">Ok</span> &mdash; All Ok<br/>
            <span class="warning">Warning</span> &mdash; Not a deal breaker, but it's recommended to have this installed
            for some features to work<br/>
