            ...
            @foreach($categorys as $category)
                ...
                <div class="col-md-6">
                    <section class="box-categories">
                        ...
                        <ul class="fa-ul">
                            ...
                            @forelse($article as $arti)
                                <li>
                                    ...
                                    <h3 class="h5"><a href="#"><a
                                                    href="{{url('show/'.$arti->slug)}}">{{$arti->name}}</a></h3>
                                    <span class="article-meta">{{$arti->created_at->format('l, d-m-Y')}}</span>
                                    <?php
                                    $str = $arti->description;
                                    $len = strlen($str);

                                    $excerpt = App\Http\Controllers\Client\kb\UserController::getExcerpt($str,
                                        $startPos = 0, $maxLength = 20);
                                    ?>
                                    {!! strip_tags($excerpt) !!} <br/><a class="more-link text-center"
                                                                         href="{{url('show/'.$arti->slug)}}"
                        ...
                        <p class="more-link text-center"><a href="{{url('category-list/'.$category->slug)}}"
                                                            class="btn btn-custom btn-xs">{!! Lang::get('lang.view_all') !!}</a>
                        </p>
                    </section>
                </div>
            @endforeach
        ...
        <section class="section">
            <div class="banner-wrapper banner-horizontal clearfix">
                ...
                <div class="banner-content">
                    <p>{!! Lang::get('lang.if_you_did_not_find_an_answer_please_raise_a_ticket_describing_the_issue') !!}
                        .</p>
                </div>
                <p><a href="{!! URL::route('form') !!}"
                      class="btn btn-custom">{!! Lang::get('lang.submit_a_ticket') !!}</a></p>
            </div>
        </section>
    </div>
@stop
