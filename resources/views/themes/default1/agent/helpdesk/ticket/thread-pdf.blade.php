...
<table>
    <tr>
        <td><b>{!! Lang::get('lang.ticket_number') !!}:</b></td>
        <td>{{$thread->ticket_number}}</td>
    </tr>
    <tr>
        <td><b>{!! Lang::get('lang.first_name') !!}:</b></td>
        <td>{{ucfirst($thread->first_name)}}</td>
    </tr>
    <tr>
        <td><b>{!! Lang::get('lang.last_name') !!}:</b></td>
        <td>{{ucfirst($thread->last_name)}}</td>
    </tr>
    <tr>
    ...
    @if($ticket->getAssignedTo())
        <tr>
            <td><b>{!! Lang::get('lang.assigned_to') !!}:</b></td>
            <td>{{$ticket->getAssignedTo()->first_name}} {{$ticket->getAssignedTo()->last_name}}</td>
        </tr>
    @endif
</table>
<hr>
            ...
            @foreach($ticket->extraFields() as $ticket_form_data)
                @if($ticket_form_data->isHidden()==false)
                    <tr style="border: 1px solid black;border-collapse: collapse;">
                        <td style="border: 1px solid black;border-collapse: collapse; padding: 14px;">
                            &nbsp;&nbsp;<b>{!! $ticket_form_data->getFieldKeyLabel() !!}</b></td>
