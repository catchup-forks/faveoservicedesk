...
//$inbox = App\Model\helpdesk\Ticket\Tickets::all();
            $myticket = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', Auth::user()->id)->where('status',
                '1')->get();
            $unassigned = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=', null)->where('status', '=',
                '1')->get();
...
//$inbox = App\Model\helpdesk\Ticket\Tickets::where('dept_id','',Auth::user()->primary_dpt)->get();
            $myticket = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', Auth::user()->id)->where('status',
                '1')->get();
            $unassigned = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=', null)->where('status', '=',
                '1')->where('dept_id', '=', Auth::user()->primary_dpt)->get();
            $tickets = App\Model\helpdesk\Ticket\Tickets::where('status', '1')->where('dept_id', '=',
        ...
        if (Auth::user()->role == 'admin' || Auth::user()->role == 'agent') {
            $todaytickets = count(App\Model\helpdesk\Ticket\Tickets::where('status', '=',
                1)->whereRaw('date(duedate) = ?', [date('Y-m-d')])->get());
        } else {
            $dept = App\Model\helpdesk\Agent\Department::where('id', '=', Auth::user()->primary_dpt)->first();
            $todaytickets = count(App\Model\helpdesk\Ticket\Tickets::where('status', '=',
                1)->whereRaw('date(duedate) = ?', [date('Y-m-d')])->where('dept_id', '=', $dept->id)->get());
        }
        ?>
            ...
            <a href="{!! route('inbox.ticket') !!}">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number"><?php echo count($tickets); ?>
                            <small> {!! Lang::get('lang.tickets') !!}</small>
                    </div>
                    <!-- /.info-box-content -->
                </div>
            </a>
            ...
            <a href="{!! route('unassigned') !!}">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{count($unassigned) }}
                            <small> {!! Lang::get('lang.tickets') !!}</small></span>
            ...
            <a href="{!! route('overdue.ticket') !!}">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{ $overdue_ticket }}
            ...
            <a href="{!! route('myticket.ticket') !!}">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{count($myticket) }}
                            <small> Tickets</small></span>
                    </div><!-- /.info-box-content -->
                </div><!-- /.info-box -->
            </a>
            ...
            <a href="{!! route('ticket.duetoday') !!}">
                <div class="info-box">
                    ...
                    <div class="info-box-content">
                        ...
                        <span class="info-box-number">{{ $todaytickets }}
                            <small> Tickets</small></span>
                    </div><!-- /.info-box-content -->
                </div><!-- /.info-box -->
            </a>
        ...
        <div class="box-body">
            <table class="table table-hover table-bordered">
                ...
                @foreach($departments as $department)
                    <?php
                    $open = App\Model\helpdesk\Ticket\Tickets::where('dept_id', '=', $department->id)->where('status',
                        '=', 1)->count();
                    $resolve = App\Model\helpdesk\Ticket\Tickets::where('dept_id', '=',
                        $department->id)->where('status', '=', 2)->count();
                    $close = App\Model\helpdesk\Ticket\Tickets::where('dept_id', '=', $department->id)->where('status',
                        '=', 3)->count();
                    $delete = App\Model\helpdesk\Ticket\Tickets::where('dept_id', '=', $department->id)->where('status',
                        '=', 5)->count();
                    ?>
                    <tr>
