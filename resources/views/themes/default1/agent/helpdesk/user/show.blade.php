...
@section('content')
    ...
    <?php $table = \Datatable::table()
        ->addColumn(
            "", Lang::get('lang.subject'), Lang::get('lang.ticket_id'), Lang::get('lang.priority'),
            Lang::get('lang.from'), Lang::get('lang.assigned_to'), Lang::get('lang.last_activity'),
                ...
                @if($users->is_delete != '1')
                    @if($users->role=='user')
                        <div class="box-footer">
                            <div id="refresh-org">
                                <?php
                                $user_org = App\Model\helpdesk\Agent_panel\User_org::where('user_id', '=',
                                    $users->id)->first();
                                ?>
                                @if($user_org == null)
                                    ...
                                    <a href="" class="pull-right" data-toggle="modal" data-target="#assign"><i
                                                ...
                                                style="color:orange;"> </i> {!! Lang::get('lang.assign') !!} </a>
                                    <a href="" data-toggle="modal" data-target="#create_org"
                                       class="pull-right"> {{Lang::get('lang.create')}} |&nbsp;</a>
                                @endif
                                @if($user_org != null)
                                    <?php
                                    $org_id = $user_org->org_id;
                                    $organization = App\Model\helpdesk\Agent_panel\Organization::where('id', '=',
                                        $org_id)->first();
                                    ?>
                                    <b>{!! Lang::get('lang.organization') !!}</b>
                                    ...
                                    <a href="" data-toggle="modal" data-target="#editassign"
                                       title="{{$organization->name}}"> <span
                                                style="color:green;">{{str_limit($organization->name,15)}}</span> </a>


                                    <a class="pull-right" href="#" data-toggle="modal" data-target="#{{$org_id}}delete"
                                       title="{!! Lang::get('lang.remove') !!}"><i class="fa fa-times"
                                                                                   style="color:red;"> </i></a>
                    ...
                    <div class="box-footer">
                        ...
                        <a class="pull-right">
                            @if($users->active == '1')
                                <span style="color:green;"> <span class="glyphicon glyphicon-ok-circle"></span>  <span
                                            class="glyphicon glyphicon-user"></span></span>
                            @else
                                <span style="color:red;"><span class="glyphicon glyphicon-ban-circle"></span><span
                                            class="glyphicon glyphicon-user"></span></span>
                            @endif
                        </a>
                    </div>
     ...
     -->
                                ...
                                @if($user->role == 'agent')
                                    {!! Form::label('delete_all_content',Lang::get('lang.delete_all_content')) !!} <span
                                            class="text-red"> *</span>
                                    <?php
                                    $open = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=',
                                        $users->id)->where('status', '=', '1')->get();
                                    ?>
                                    <?php $user = App\User::where('id', $users->id)->first(); ?>
                                    <?php
                                    $open = count(App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=',
                                        $users->id)->where('status', '=', '1')->get());
                                    ?>
                            ...
                            <!--    Hi Admin 
                                ...
                                $open = count(App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=',
                                    $users->id)->where('status', '=', '1')->get());
                                $counted = count(App\Model\helpdesk\Ticket\Tickets::where('user_id', '=',
                                    $users->id)->where('status', '=', '3')->get());
                                $deleted = count(App\Model\helpdesk\Ticket\Tickets::where('user_id', '=',
                                    $users->id)->where('status', '=', '5')->get());
                                ?>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <!-- <div class="col-xs-4 form-group {{ $errors->has('primary_department') ? 'has-error' : '' }}"> -->
                            {!! Form::label('primary_dpt',Lang::get('lang.primary_department')) !!} <span
                                    class="text-red"> *</span>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <button class="btn btn-warning pull-right"
                                id="changepassword">{{Lang::get('lang.password_generator')}}</button>

                        <br/>
                        <form name="myForm" action="{!!URL::route('user.post.changepassword', $users->id)!!}"
                              method="post" role="form" onsubmit="return validateForm()">
                            <div class="form-group">

                            <!-- <div class="form-group {{ $errors->has('change_password') ? 'has-error' : '' }}"> -->
                                {!! Form::label('New password',Lang::get('lang.new_password')) !!} <span
                                        class="text-red"> *</span>
        ...
        <div class="col-md-9">
            ...
            if ($users->role != 'user') {
                $open = count(App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=', $users->id)->where('status',
                    '=', '1')->get());
                $tickets = App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=', $users->id)->where('status',
                    '=', '3')->orderBy('id', 'DESC')->paginate(20);
                $counted = count(App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=',
                    $users->id)->where('status', '=', '3')->get());
                $deleted = count(App\Model\helpdesk\Ticket\Tickets::where('assigned_to', '=',
                    $users->id)->where('status', '=', '5')->get());
            }
            if ($users->role == 'user') {
                $open = count(App\Model\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=',
                    '1')->get());
                $tickets = App\Model\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status', '=',
                    '3')->orderBy('id', 'DESC')->paginate(20);
                $counted = count(App\Model\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status',
                    '=', '3')->get());
                $deleted = count(App\Model\helpdesk\Ticket\Tickets::where('user_id', '=', $users->id)->where('status',
                    '=', '5')->get());
                // dd($deleted);
            }
            ?>
            <div class="row">
                            ...
                            <div style="margin-left:1%;">
                                @if($users->is_delete != '1')
                                    @if(Auth::user()->role == 'admin')
                                        ...
                                        @if($users->role == 'user')
                                            <div class="btn-group">
                                                <button type="button" href="#myPopup" data-rel="popup"
                                    ...
                                    @if(Auth::user()->role == 'admin')
                                        <a href="{{route('user.edit', $users->id)}}">
                                            <button type="button" href="{{route('user.edit', $users->id)}}"
                                                    class="btn btn-primary btn-sm">{{Lang::get('lang.edit')}}</button>
                                        </a>
                                        <button type="button" href="#myPopup" data-rel="popup"
                                                class="btn btn-primary btn-sm" data-toggle="modal"
                                    ...
                                    @if(Auth::user()->role == 'admin')
                                        <div id="page" class="hfeed site">
                                            <article class="hentry error404 text-center">
                                                <h1 class="error-title"><i class="fa fa-trash text-info"
                                                                           style="color: grey"></i><span
                                                            class="visible-print text-danger">0</span></h1>
                                                ...
                                                <div class="entry-content clearfix">
                                                    ...
                                                    <p>
                                                        <button type="button" href="#myPopup" data-rel="popup"
                                                                ...
                                                                data-target="#addNewCategoryModal8">{{Lang::get('lang.restore-user')}}</button>
                                                        </a></p>
                                                </div><!-- .entry-content -->
                                            </article><!-- .hentry -->
                                        </div><!-- #page -->
                            ...
                            @if($users->is_delete != '1')
                                <div style="padding:1%;">
                                    <ul class="nav nav-tabs">
                                        ...
                                        <li class="active"><a href="#tab_1" id="open_tab"
                                                              data-toggle="tab">{!! Lang::get('lang.open_tickets') !!}
                                                ({{$open}})</a></li>
                                        <li><a href="#tab_2" id="closed_tab"
                                               data-toggle="tab">{!! Lang::get('lang.closed_tickets') !!} ({{$counted}}
                                                )</a></li>
                                        <li><a href="#tab_3" id="deleted_tab"
                                               data-toggle="tab">{!! Lang::get('lang.deleted_tickets') !!} ({{$deleted}}
                                                )</a></li>
                                    </ul>
                                    ...
                                    <!-- failure message -->
                                        ...
                                        <div class="mailbox-controls">
                                            <!-- Check all button -->
                                            <a class="btn btn-default btn-sm checkbox-toggle"><i
                                                        class="fa fa-square-o"></i></a>
                                            <input type="submit" class="btn btn-default text-orange btn-sm"
                                                   name="submit" value="{!! Lang::get('lang.delete') !!}">
                                            <input type="submit" class="btn btn-default text-yellow btn-sm"
                                                   name="submit" value="{!! Lang::get('lang.close') !!}">
                                            <div class="pull-right">
                                            </div>
                                            <!--</div>-->
                                            <div id="more-option" class="btn-group">
                                                <button type="button" class="btn btn-sm btn-default dropdown-toggle"
                                                        data-toggle="dropdown" id="d2">
                                                    <i class="fa fa-reorder" style="color:teal;"> </i>
                                                    {!! Lang::get('lang.sort-by') !!} <span class="caret"></span>
                                                </button>
                                                <ul class="dropdown-menu pull-right">
                                                    <li data-toggle="modal" data-target="#ChangeOwner"><a href="#"
                                                                                                          class="toggle-vis"
                                                                                                          data-column="7"><i
                                                                    class="fa fa-plus-square-o"
                                                                    style="color:green;"> </i>{!!Lang::get('lang.created-at')!!}
                                                        </a></li>
                                                </ul>
                                            </div>
                                        </div>
                                        ...
                                        <div class="box-body mailbox-messages" id="refresh">
                                            <p style="display:none;text-align:center; position:fixed; margin-left:40%;margin-top:-70px;"
                                               id="show" class="text-red"><b>{!! Lang::get('lang.loading') !!}...</b>
                                            </p>
                                            <!-- table -->
                ...
                @if($users->is_delete != '1')
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <link type="text/css"
                                  href="{{asset("lb-faveo/css/bootstrap-datetimepicker4.7.14.min.css")}}"
                                  rel="stylesheet">
                            <div class="box box-info">
                                ...
                                <div class="box-body">
                                    <form id="foo">
                                        <div class="form-group">
                                            <div class="row">
                                                ...
                                                <?php
                                                $start_date = App\Model\helpdesk\Ticket\Tickets::where('id', '=',
                                                    '1')->first();
                                                if ($start_date != null) {
                                                    ...
                                                    $start_date = date("m/d/Y",
                                                        strtotime($created_date . ' -1 months'));
                                                } else {
                                                    $start_date = date("m/d/Y",
                                                        strtotime(date("m/d/Y") . ' -1 months'));
                                                }
                                                ?>
                                                ...
                                                <div class='col-sm-3'>
                                                    ...
                                                    <input type="submit" value="{!! Lang::get('lang.submit') !!}"
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        @if($users->role=='user')
                                                            ...
                                                            <div class="col-md-4"><span id="legend-holder"
                                                                                        style="background-color: #6C96DF;"></span>&nbsp;
                                                        ...
                                                        @if($users->role=='agent')
                                                            <div class="col-md-4"><span id="legend-holder"
                                                                                        style="background-color: #6C96DF;"></span>&nbsp;
                                                                <span class="lead"> <span
                                                                            id="total-created-tickets"></span> {!! Lang::get('lang.assign_tickets') !!}  </span>
                                                            </div>
                                                        ...
                                                        <div class="col-md-4"><span id="legend-holder"
                                                                                    style="background-color: #6DC5B2;"></span>&nbsp;
                                                            <span class="lead"> <span id="total-reopen-tickets"
                                                                                      class="lead"></span> {!! Lang::get('lang.tickets') !!} {!! Lang::get('lang.reopen') !!}  </span>
                                                        </div>
                                                        <div class="col-md-4"><span id="legend-holder"
                                                                                    style="background-color: #E3B870;"></span>&nbsp;
                                                            <span class="lead"> <span id="total-closed-tickets"
                                                                                      class="lead"></span> {!! Lang::get('lang.tickets') !!} {!! Lang::get('lang.closed') !!}  </span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="chart">
                                        <canvas class="chart-data" id="tickets-graph" width="1000"
                                                height="250"></canvas>
                                    </div>
                                </div><!-- /.box-body -->
                            </div><!-- /.box -->
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="alert alert-danger alert-dismissable" id="alert-danger" style="display:none;">
                                <i class="fa fa-ban"> </i> <b> {!! Lang::get('lang.alert') !!} ! <span
                                            id="get-danger"></span> </b>
                            ...
                            <div class="row" id="hide">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <spam id="error-name" style="display:none;position:fixed"
                                              class="call-out text-red">This is a required field
                                        </spam>
                                        <spam id="error-name1" style="display:none;position:fixed"
                                              class="call-out text-red">! Allready Taken
                                        </spam>
                                        <br/>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="url" name="website" placeholder="https://www.example.com"
                                               class="form-control">
                                        <spam id="error-website" style="display:none" class="help-block text-red">!
                                            Allready Taken
                                        </spam>
                                        <br/>
                                    </div>
            ...
            <div class="modal fade" id="assign">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" id="dismiss" aria-label="Close">
                        ...
                        <div id="assign_alert" class="alert alert-success alert-dismissable" style="display:none;">
                            <button id="assign_dismiss" type="button" class="close" data-dismiss="alert"
                                    aria-hidden="true">×
                            </button>
                ...
                <div class="modal fade" id="editassign">
                    <div class="modal-dialog">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" id="editdismiss"
                            ...
                            <div class="modal-body">
                                ...
                                <div id="assign_body">
                                    ...
                                    <input type="text" id="editorg" class="form-control" name="org"
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default pull-left" data-dismiss="modal"
                                        id="dismis4">{!! Lang::get('lang.close') !!}</button>
                                <button type="submit" class="btn btn-success pull-right"
                                        id="submt3">{!! Lang::get('lang.assign') !!}</button>
                            </div>
                            {!! Form::close()!!}
                        </div><!-- /.modal-content -->
                    </div><!-- /.modal-dialog -->
            ...
            <script type="text/javascript">
                $(document).ready(function () {
                    $.getJSON("../user-agen/<?php echo $users->id; ?>", function (result) {
                        var labels = [], open = [], closed = [], reopened = [], open_total = 0, closed_total = 0,
                            reopened_total = 0;
                    ...
                    $('#foo').submit(function (event) {
                        ...
                        $.ajax({
                            ...
                            success: function (result2) {
                                ...
                                var labels = [], open = [], closed = [], reopened = [], open_total = 0,
                                    closed_total = 0, reopened_total = 0;
            ...
            <script src="{{asset("lb-faveo/js/bootstrap-datetimepicker4.7.14.min.js")}}"
